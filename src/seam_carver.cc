 ...
 * @brief Namespace containing classes and functions for image seam carving.
 */

namespace seam_carving {

const float SeamCarver::s_removal_energy_val_ = -100.f; // removal energy value
const float SeamCarver::s_protection_energy_val_ = 100.f; // protection energy value

SeamCarver::SeamCarver(const cv::Mat &input_image) {
  origin_image_ = input_image.clone();  // copy the input image
  carved_image_ = input_image.clone();
  removal_region_mask_ = cv::Mat::zeros(
      origin_image_.rows, origin_image_.cols, CV_8UC1); // init removal mask
  protection_region_mask_ = cv::Mat::zeros(
      origin_image_.rows, origin_image_.cols, CV_8UC1); // init protection mask
}
...
void SeamCarver::VerticalCarving(int num_seams) {
  for (int i = 0; i < num_seams; ++i) {
...
#ifdef VIZ_DEBUG
    cv::Mat seam_image = carved_image_.clone(); // visualization for debugging or anything
    ...
    cv::imshow("Seam Image", seam_image); // visualization for debugging or anything
    cv::waitKey(1);
#endif
...
/**
 * Finds the vertical seam with the lowest energy in the image.
 * 
 * @param seam Pointer to a vector that will store the indices of the pixels in the seam.
 */
...
/**
 * Finds the seam with dynamic programming algorithm.
 * 
 * @param energy_map The energy map of the image.
 * @param seam[out] The vector to store the seam indices.
 */
...
/**
 * Removes a horizontal seam from the carved image.
 * 
 * @param seam The indices of the pixels in the seam to be removed.
 */
...
/**
 * @brief Removes a vertical seam from the carved image.
 * 
 * This function removes a vertical seam from the carved image, based on the provided seam vector.
 * The function updates the carved image, removal region mask, and protection region mask accordingly.
 * 
 * @param seam The vector containing the indices of the pixels to be removed in each row.
 */
void SeamCarver::RemoveVerticalSeam(const std::vector<int> &seam) {
  ...
  carved_image_ = carved_img.clone(); // copy the carved image
  removal_region_mask_ = carved_removal_mask.clone(); // copy the carved removal mask
  protection_region_mask_ = carved_protection_mask.clone(); // copy the carved protection mask
}
